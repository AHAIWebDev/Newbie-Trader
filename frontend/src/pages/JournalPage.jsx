...
/**
 * Full trade journal page.
 *
 * Shows performance stats at the top and all trades below,
 * newest first. Each trade is expandable to show full reasoning
 * and market context.
 */
export default function JournalPage() {
  const [trades,  setTrades]  = useState([]);
  const [filter,  setFilter]  = useState('all');   // all | buy | sell | win | loss
  const [search,  setSearch]  = useState('');

  // Load trades from localStorage (same store as Portfolio)
  useEffect(() => {
    try {
      const saved    = localStorage.getItem(STORAGE_KEY);
      const portfolio = saved ? JSON.parse(saved) : null;
  ...
  // Update outcome notes in localStorage when edited
  const handleUpdateNotes = (tradeId, notes) => {
    setTrades(prev => {
      const updated = prev.map(t =>
        t.id === tradeId ? { ...t, outcomeNotes: notes } : t
      );
  ...
  const winRate     = sells.length > 0
    ? ((wins.length / sells.length) * 100).toFixed(0)
    : null;
  ...
  const bestTrade   = sells.reduce(
    (best, t) => (!best || t.pnl > best.pnl ? t : best), null
  );
  const worstTrade  = sells.reduce(
    (worst, t) => (!worst || t.pnl < worst.pnl ? t : worst), null
  );
